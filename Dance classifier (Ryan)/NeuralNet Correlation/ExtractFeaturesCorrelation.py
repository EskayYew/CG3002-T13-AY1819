...
def extractFeatures(data):
    ...
    for i in range(len(readings)):
        ...
        if (i == 18): #Omit flex sensor from correlation
            continue
        #Calculate standard deviation first. IT IS NOT USED AS A FEATURE.
        standardDeviation1 = np.std(axis1)

        for j in range(len(readings)-1): #Omit flex sensor from correlation
            if (i == j):
                continue
